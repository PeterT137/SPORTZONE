...
const RegisterForm: React.FC<RegisterFormProps> = ({ role }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Example: await fetch(`/api/register-${role}`, { method: 'POST', body: JSON.stringify(formData) });
  };
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit} noValidate>
      ...
      <div>
        <label
          htmlFor={`register-${role}-password`}
          className="block text-sm font-medium text-gray-500 mb-1"
        >
          Password
        </label>
      ...
      <div>
        <label
          htmlFor={`register-${role}-password-confirm`}
          className="block text-sm font-medium text-gray-500 mb-1"
        >
          Confirm Password
        </label>
