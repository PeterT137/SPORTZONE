...
const FacilityManager: React.FC = () => {
    ...
    const [facilityToDelete, setFacilityToDelete] = useState<number | null>(
        null
    );
    ...
    const [formData, setFormData] = useState<
        Omit<Facility, "fac_id" | "fields" | "services"> & {
            imageUrls: string[];
        }
    >({
        userId: 0,
    ...
    const fetchFacilities = async (searchText: string = "") => {
        ...
        try {
            ...
            showToast(
                "Không thể lấy danh sách cơ sở. Vui lòng thử lại.",
                "error"
            );
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        formDataPayload.append(
            "CloseTime",
            convertTo24Hour(formData.close_time)
        );
        ...
        if (editId !== null) {
            ...
            console.log(
                "Original existing URLs from imagePreviews:",
                existingUrls
            );
            ...
            const originalFacility = facilities.find(
                (f) => f.fac_id === editId
            );
        ...
        try {
            ...
            if (response.status === 204) {
                ...
                const mappedFacility: Facility = {
                    ...
                    close_time: convertTo24Hour(formData.close_time).slice(
                        0,
                        5
                    ),
                ...
                if (editId !== null) {
                    setFacilities((prev) =>
                        prev.map((f) =>
                            f.fac_id === editId ? mappedFacility : f
                        )
                    );
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (userStr) {
                ...
                if (
                    user.FieldOwner &&
                    Array.isArray(user.FieldOwner.facilities)
                ) {
                    user.FieldOwner.facilities =
                        user.FieldOwner.facilities.filter(
    ...
    const handleViewDetails = async (facility: Facility) => {
        ...
        try {
            ...
            showToast(
                "Không thể lấy chi tiết cơ sở. Vui lòng thử lại.",
                "error"
            );
            setError(err instanceof Error ? err.message : "Lỗi không xác định");
        }
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-600 mt-1">
                                        Quản lý và theo dõi các cơ sở thể thao
                                        của bạn
                                    </p>
                                </div>
                            </div>
