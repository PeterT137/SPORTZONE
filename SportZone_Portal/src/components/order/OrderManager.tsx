...
export function OrderManagement() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setOrderServices([]); // will be fetched per order on demand
    ...
    () => ({
      ...
      async updateOrderContentPayment(orderId: number, option: number) {
        ...
        if (!res.ok) {
          ...
          throw new Error(
            text || `Update content payment failed: ${res.status}`
          );
        }
        const json = await res.json();
        return json.data ?? json;
      ...
      async getScheduleDetail(scheduleId: number) {
        const res = await fetch(`${API_URL}/api/Order/schedule/${scheduleId}`, {
          method: "GET",
          headers: { "Content-Type": "application/json", ...getAuthHeaders() },
        });
      ...
      async getScheduleSlotDetail(scheduleId: number) {
        const res = await fetch(
          `${API_URL}/api/Order/schedule/${scheduleId}/slot-detail`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!res.ok) throw new Error(`Get slot detail failed: ${res.status}`);
        const json = await res.json();
        return json.data ?? json;
      },
    }),
  ...
  useEffect(() => {
    (window as any).__orderApis = api;
    return () => {
      try {
        delete (window as any).__orderApis;
      } catch {
        // ignore
      }
    };
  ...
  const updateOrderService = async (
    ...
    try {
      const updatedOrderService = await OrderService.updateOrderService(
        orderServiceId,
        updates
      );
      setOrderServices((prev) =>
        prev.map((os) =>
          os.order_service_id === orderServiceId ? updatedOrderService : os
        )
      );
      const orderService = orderServices.find(
        (os) => os.order_service_id === orderServiceId
      );
      if (orderService) {
        await OrderService.updateOrderTotal(orderService.order_id);
        const updatedOrders = await OrderService.getOrders();
  ...
  const deleteOrderService = async (orderServiceId: string) => {
    try {
      ...
      setOrderServices((prev) =>
        prev.filter((os) => os.order_service_id !== orderServiceId)
      );
      if (orderService) {
        await OrderService.updateOrderTotal(orderService.order_id);
        const updatedOrders = await OrderService.getOrders();
  ...
  const bookAdditionalHours = async (
    orderId: string,
    hours: number,
    pricePerHour: number
  ) => {
  ...
  const confirmPayment = async (
    ...
    try {
      ...
      // Optionally fetch fresh order detail from backend if numeric id
      const numericId = Number(orderId);
      if (!Number.isNaN(numericId)) {
        try {
          const fresh = await api.getOrderById(numericId);
          console.log("Fresh backend order detail:", fresh);
        } catch (e) {
          console.warn("Fetch backend order detail failed:", e);
        }
      }
    } catch (error) {
  ...
  const stats = {
    ...
    totalRevenue: orders
      .filter((o) => o.status_payment === "paid")
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-500">
              Quản lý đơn hàng và dịch vụ của khách hàng
            </p>
          </div>
        </div>
