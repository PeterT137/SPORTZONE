...
export function OrderDetailModal({
  ...
  console.log(
    `OrderDetailModal - Order ID: ${order.order_id}, Status: ${order.status_payment}, User Role: ${user.role}`
  ); // Debug log
  ...
  const handlePayment = async () => {
    if (canModify && !isPaying) {
      ...
      console.log(
        `Cannot confirm payment: canModify=${canModify}, isPaying=${isPaying}`
      ); // Debug log
    }
  };
  ...
  const handleCancel = async () => {
    if (
      ...
      console.log(
        `Cannot cancel: status=${order.status_payment}, role=${user.role}, isCancelling=${isCancelling}`
      ); // Debug log
    }
  };
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500">
                        {orderService.service_description}
                      </p>
                      <div className="flex items-center gap-4 mt-2 text-sm">
                        <span>Số lượng: {orderService.quantity}</span>
                        <span>
                          Đơn giá: {orderService.price.toLocaleString("vi-VN")}đ
                        </span>
                        <span className="font-medium">
                          Thành tiền:{" "}
                          {(
                            orderService.quantity * orderService.price
                    ...
                    {canModify && (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            onDeleteService(orderService.order_service_id)
                          }
                          title="Xóa dịch vụ"
                        >
